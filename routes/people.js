...
router.post("/save", uploading.single("Photo"), async (req, res) => {
...
router.get("/fetch", async (req, res) => {
  try {
    let fetchData = await People.find();
...
router.get("/fetch/:id", async (req, res) => {
    const { id } = req.params;
    try {
      let fetchData = await People.findById(id);
      return res.status(200).send(fetchData);
    } catch (error) {
      res.status(404).json({ message: error.message });
  ...
  router.put(
    ...
    async (req, res) => {
      ...
      await People.findByIdAndUpdate(id, Data);
